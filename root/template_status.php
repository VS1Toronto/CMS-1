...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	if($author == $log_username || $account_name == $log_username ){
		$statusDeleteButton = '<span id="sdb_'.$statusid.'"><a href="#" onclick="return false;" onmousedown="deleteStatus(\''.$statusid.'\',\'status_'.$statusid.'\');" title="DELETE THIS STATUS AND ITS REPLIES" style="color:#858585">delete status</a></span> &nbsp; &nbsp;';
	}
    ...
    if($replynumrows > 0){
        while ($row2 = mysqli_fetch_array($query_replies, MYSQLI_ASSOC)) {
			...
			if($replyauthor == $log_username || $account_name == $log_username ){
				$replyDeleteButton = '<span id="srdb_'.$statusreplyid.'"><a href="#" onclick="return false;" onmousedown="deleteReply(\''.$statusreplyid.'\',\'reply_'.$statusreplyid.'\');" title="DELETE THIS COMMENT" style="color:#858585">remove</a></span>';
			}
	...
	$statuslist .= '<div id="status_'.$statusid.'" class="status_boxes"><div><b>Posted by <a style="color: #858585;"  href="user.php?u='.$author.'">'.$author.'</a> '.$postdate.':</b> '.$statusDeleteButton.' <br />'.$data.'</div>'.$status_replies.'</div>';
...
function postToStatus(action,type,user,ta){
	var data = _(ta).value;
	if(data == ""){
		alert("You would hve to Type something first");
		return false;
	}
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			...
			if(datArray[0] == "reply_ok"){
				var rid = datArray[1];
				data = data.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br />").replace(/\r/g,"<br />");
				_("status_"+sid).innerHTML += '<div id="reply_'+rid+'" class="reply_boxes color:#858585"><div><b>Reply by you just now:</b><span id="srdb_'+rid+'"><a href="#" onclick="return false;" onmousedown="deleteReply(\''+rid+'\',\'reply_'+rid+'\');" title="DELETE THIS COMMENT"" style="color:#858585>remove</a></span><br />'+data+'</div></div>';
